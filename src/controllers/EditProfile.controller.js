...
async function EditProfile(req, res) {
  try {
    ...
    if (locationChanged) {
      ...
      // Check if this is the first member of this family in the new location
      const familyInNewLocation = await prisma.peopleRegistry.findFirst({
        where: {
          PR_STATE_CODE: newStateCode,
          PR_DISTRICT_CODE: newDistrictCode,
          PR_CITY_CODE: Number(newCityCode),
          PR_MOBILE_NO: mobileNumber,
          NOT: { PR_ID: Number(PR_ID) }, // Exclude current profile
        },
      });

      if (familyInNewLocation) {
        // Family already exists in this location, use their family number
        familyNumber = familyInNewLocation.PR_FAMILY_NO;
      } else {
        // This is a new family in this location, get next available family number
      ...
      // If this is the first member with this mobile number
      if (familyMembers.length === 0) {
        memberNumber = "001";
      } else {
        // Determine member's position within family
        const memberIndex = familyMembers.findIndex(
          (m) => m.PR_ID === Number(PR_ID)
        );

        if (memberIndex >= 0) {
          // Existing member - keep their sequence in the family
          memberNumber = (memberIndex + 1).toString().padStart(3, "0");
        } else {
          // New member - add to end of family
          memberNumber = (familyMembers.length + 1).toString().padStart(3, "0");
        }
      }

      // Update location-based data for this member
      updateData.PR_UNIQUE_ID = `${newStateCode}${newDistrictCode}-${newCityCode}-${familyNumber}-${memberNumber}`;
      updateData.PR_STATE_CODE = newStateCode;
      updateData.PR_DISTRICT_CODE = newDistrictCode;
      updateData.PR_CITY_CODE = Number(newCityCode);
      ...
      // Also update all other members of this family if they're moving together
      if (familyMembers.length > 1) {
        await Promise.all(
          familyMembers
            .filter((member) => member.PR_ID !== Number(PR_ID))
            .map(async (member, idx) => {
              const otherMemberNumber = (idx + 1).toString().padStart(3, "0");

              await prisma.peopleRegistry.update({
                where: { PR_ID: member.PR_ID },
                data: {
                  PR_UNIQUE_ID: `${newStateCode}${newDistrictCode}-${newCityCode}-${familyNumber}-${otherMemberNumber}`,
                  PR_STATE_CODE: newStateCode,
                  PR_DISTRICT_CODE: newDistrictCode,
                  PR_CITY_CODE: Number(newCityCode),
                  PR_FAMILY_NO: familyNumber,
                  PR_MEMBER_NO: otherMemberNumber,
                  PR_UPDATED_AT: new Date(),
                },
              });
            })
