...
async function EditProfile(req, res) {
  try {
    ...
    // Check if city information changed
    const cityChanged =
      req.body.PR_CITY_NAME !== existingProfile.PR_CITY_NAME ||
      req.body.PR_DISTRICT_CODE !== existingProfile.PR_DISTRICT_CODE ||
      req.body.PR_STATE_CODE !== existingProfile.PR_STATE_CODE;

    let cityCode = existingProfile.PR_CITY_CODE;
    let cityName = existingProfile.PR_CITY_NAME;

    if (cityChanged) {
      // Find or create the new city
      let city = await prisma.city.findFirst({
        where: {
          CITY_NAME: req.body.PR_CITY_NAME,
          CITY_DS_CODE: req.body.PR_DISTRICT_CODE,
          CITY_ST_CODE: req.body.PR_STATE_CODE,
        },
      });
      ...
      if (!city) {
        ...
        city = await prisma.city.create({
          data: {
            ...
            CITY_NAME: req.body.PR_CITY_NAME,
            CITY_DS_CODE: req.body.PR_DISTRICT_CODE,
            CITY_DS_NAME: req.body.PR_DISTRICT_NAME,
            CITY_ST_CODE: req.body.PR_STATE_CODE,
            CITY_ST_NAME: req.body.PR_STATE_NAME,
            CITY_PIN_CODE: req.body.PR_PIN_CODE,
          },
        });
      }
    ...
    // Generate new unique ID if location changed
    let uniqueId = existingProfile.PR_UNIQUE_ID;
    if (cityChanged) {
      const timestamp = Date.now().toString().slice(-4);
      const randomSuffix = Math.floor(100 + Math.random() * 900);
      uniqueId = `${req.body.PR_STATE_CODE}${req.body.PR_DISTRICT_CODE}-${cityCode}-${timestamp}-${randomSuffix}`;
    }
