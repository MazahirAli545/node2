...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, "./uploads/"); // Save to a local uploads folder
  },
...
const upload = multer({ storage }).single("image"); // Expecting file field named 'image'
  ...
  try {
    ...
    upload(req, res, async (err) => {
      ...
      const updatedProfile = await prisma.peopleRegistry.update({
        ...
        data: {
          ...req.body,
          PR_PHOTO_URL, // Set updated photo URL if available
          PR_UPDATED_AT: new Date(),
        },
      });
