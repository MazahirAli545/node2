...
async function EditProfile(req, res) {
  try {
    ...
    // NEW LOGIC: Handle Parent ID priority (Father ID first, then Mother ID)
    const fatherID = convertToNumberOrNull(req.body.PR_FATHER_ID);
    ...
    if (parentID && existingProfile.PR_IS_COMPLETED !== "Y") {
      try {
        ...
        if (parentProfile && parentProfile.PR_UNIQUE_ID) {
          // Extract components from parent's PR_UNIQUE_ID
          // Format: STATE_CODE + DISTRICT_CODE + "-" + CITY_CODE + "-" + FAMILY_NO + "-" + MEMBER_NO
          const parentUniqueId = parentProfile.PR_UNIQUE_ID;
          const parts = parentUniqueId.split("-");

          if (parts.length >= 4) {
            const locationPrefix = parts[0]; // STATE_CODE + DISTRICT_CODE
            const cityCode = parts[1];
            const familyNumber = parts[2];

            // Find the maximum member number in this family
            const maxMemberResult = await prisma.$queryRaw`
              SELECT MAX(CAST(SUBSTRING_INDEX(PR_UNIQUE_ID, '-', -1) AS UNSIGNED)) AS max_member
              FROM PEOPLE_REGISTRY
              WHERE PR_UNIQUE_ID LIKE CONCAT(${locationPrefix}, '-', ${cityCode}, '-', ${familyNumber}, '-%') COLLATE utf8mb4_bin
            `;

            const maxMemberNumber = maxMemberResult[0]?.max_member || 0;
            const newMemberNumber = String(maxMemberNumber + 1).padStart(
              4,
              "0"
            );

            // Generate new PR_UNIQUE_ID
            const newPrUniqueId = `${locationPrefix}-${cityCode}-${familyNumber}-${newMemberNumber}`;

            // Update the data with parent's location and new unique ID
            Object.assign(updateData, {
              PR_UNIQUE_ID: newPrUniqueId,
              PR_STATE_CODE: parentProfile.PR_STATE_CODE,
              PR_DISTRICT_CODE: parentProfile.PR_DISTRICT_CODE,
              PR_CITY_CODE: parentProfile.PR_CITY_CODE,
              PR_FAMILY_NO: familyNumber,
              PR_MEMBER_NO: newMemberNumber,
            });
          }
        }
      } catch (parentError) {
