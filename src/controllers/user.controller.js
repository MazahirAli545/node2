...
export const registerUser = async (req, res) => {
  try {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      // Business Handling
      const business = profileData.PR_BUSS_STREAM && profileData.PR_BUSS_TYPE 
      ...
      const familyNumber = existingUsers.length > 0
...
export const LoginUser = async (req, res) => {
  try {
    ...
    // OTP Check
    if (!otp) {
      const userExists = await prisma.peopleRegistry.count({ where: { PR_MOBILE_NO } });
    ...
    const users = await prisma.peopleRegistry.findMany({
      where: { PR_MOBILE_NO },
      include: { Children: true, City: true, Profession: true, BUSSINESS: true },
      orderBy: { PR_ID: "desc" }
    });
